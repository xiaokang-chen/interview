...
3. **隔离性（重点）：**
数据库允许多个并发事务同时对其数据进行读写和修改的能力，隔离性可以防止多个事务并发执行时由于交叉执行而导致数据的不一致。利用`多版本并发控制`或`锁机制`来保证；
4. **持久性：**
事务处理结束，对数据的修改是永久的，即便系统故障也不会丢失；利用`重做日志（redo log）`来保证
...
1. 显式启动事务，begin或start transaction。提交使用commit，回滚使用rollback

    ```sql
    begin;
    SELECT e from table4 WHERE a = 1;
    UPDATE table4 set e = "aa" WHERE a = 1;
    commit;
    ```

2. set autocommit=0，可以通过隐式语句默认开启事务（所有的语句都会走事务，不需要begin，但是需要commit）
    ...
    -- innodb_trx是一个用于管理InnoDB存储引擎的事务的系统表
    SELECT * FROM information_schema.innodb_trx;
    ```

TIP：

- 开发过程中建议总是`set autoconmmit=1`，通过显式语句的方式来启动事务。可以利用链式事务语法，在commit上一个事务
后自动启动下一个事务，省去begin语句的开销，也避免了隐式事务带来的**长事务**的问题；
- 不使用长事务的原因在于，为了确保事务在执行期间看到的数据在前后保持一致，那些老的read view和回滚日志就必须存在，
导致占用大量的存储空间；
...
事务在执行时，并不会一次性把所有行锁都获取到，而是执行到哪一行就拿哪一行的锁。等到commit之后，将所有锁一起释放。**所以我们一般把并发度高的更新语句放到事务末尾，以减少锁时间，提高并发度**。

```sql
begin;
update A set a=a+1 where A_id = 1;
update B set a=a+1 where B_id = 3;
-- commit之后将A id=1和B id=3对应的两行数据的锁一起释放
commit;
```

**二、死锁：**

当并发系统中不同线程出现循环资源依赖，就会导致线程无限等待，造成**死锁**。

![死锁](./pic/死锁.jpg)

解决方法：

1. 设置超时时间：`innodb_lock_wait_time`；
2. 发生死锁后主动回滚某个事务，让其他事务得以执行，开启`innodb_deadlock_detcet=on`；

超时时间通常我们不太好控制，所以一般使用第二种方案。
...
![MySQL_事务隔离级别差异](/pic/MySQL_事务隔离级别差异.png)
...
1. 如果是“读未提交”，则V1是2，这个时候虽然事务B还没有提交，但是结果已经被A提前看到，V2、V3也是2；
2. 如果是“读已提交”，则V1是1，V2是2，事务B的更新在提交后才能被A看到，所以V3也是2；
3. 如果是“可重复读”，则V1和V2是1，V3是2，在这个隔离级别下事务在执行期间看到的数据前后保持一致；
4. 如果是“串行化”，每次读写操作都会加锁，事务B在执行将1改为2时，由于此前事务A执行了读操作，这样就发生了读写冲突，此时
事务B被锁住，直到事务A提交后，事务B才能继续执行。事务B执行后再查询值V3，就是2了；
...
按加锁范围，数据库锁包括“全局锁”、“表级锁”、“行级锁”

1. 全局锁：整个库处于只读状态（Flush tables with read lock；UNLOCK TABLES;）
使用场景：全库逻辑备份，也就是整库每个表都select出来数据存成文本
建议使用FTWRL方式，而不是`set global readonly=true`，因为两者在异常处理上有差异。
当客户端发生异常中断，前者会自动释放全局锁，整个库回到正常可更新状态，而后者会一直保持readonly状态，导致整个库长时间处于不可写状态。
2. 表级锁：包括表锁和元数据锁（MDL）

    - 表锁语法：lock tables [表名] read/write；UNLOCK TABLES;
    表锁限制别的线程读写的同时，也会限制本线程接下来的操作对象：
    **比如：**

      ```sql
      -- 其他线程写t1和读t2会被阻塞
      -- 本线程只能读t1和读写t2，不能写t1和读t2
      lock tables t1 read, t2 write
      ```

    - MDL锁
    不需要显式的使用，在访问一个表的时候被自动加上。MDL的作用确保读写的正确性；
    当对一个表做增删改查的时候，会默认加上MDL读锁；对表结构做修改的时候，会默认加MDL写锁；
    **读锁之间不互斥，可以有多个线程同时对一张表进行增删改查；读写锁之间，写锁之间是互斥的，用来保证变更表结构操作的安全性，因此有两个线程要同时给一个表加字段，其中一个要等另外一个执行完才能开始执行。**

      <font color='red'>MDL锁的坑：给一个表加字段，导致整个库挂了：
      问题：事务在执行时，会默认加MDL锁。对于一个长事务，事务部提交，就会一直占着MDL锁。因为事务导致DDL语句被阻塞，而后续的大量DML语句也会被阻塞，会导致大量的客户端重试，这个库的线程很快就会爆满。
      解决方法：kill掉长事务或者暂停DDL；但是kill掉长事务未必管用，因为新的请求马上来了，理想的做法是在DDL语句里面设置超时时间，超时则不阻塞后续业务语句，后续再重新尝试获取MDL写锁</font>
