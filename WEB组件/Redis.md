...
为了实现节点从后往前遍历，后面listpack保存len也是为了相同目的。
...
数据量很大时，跳表的查找复杂度为O(logN)---**二分查找**

跳表查询过程：
查找一个跳表节点时，跳表会从头节点的最高层开始，逐一遍历每一层。在遍历某一层的跳表节点时，会用跳表节点中的 SDS 类型的元素和元素的权重来进行判断：

1. 如果当前节点权重【小于】要查找的权重时，跳表就会访问该层的下一节点；
2. 如果当前节点key的SDS数据类型【小于】要查找的数据时，跳表就会访问该层的下一节点；

如果上述两个条件都不满足，或者下一节点为空，跳表就会根据当前节点的下一层指针跳到下一层继续查找。

**zset为什么使用跳表而不是平衡树：**

1. 内存占用上：平衡二叉树每个节点占用2个指针，跳表每个节点占用1.33个指针；
2. 范围查找：zset经常使用`ZRANGE`和`ZREVRANGE`这样的范围查找命令，平衡树上找到最小值后，还需要中序遍历去找最大值，实现上不如跳表简单（跳表找到最小值后只需要对第一层链表进行若干次遍历）；
3. 插入删除：平衡二叉树插入和删除可能引发子树的调整，跳表只需要修改相邻节点的指针；
...
（1）对于并发量低且对缓存命中率高的业务使用“先更新数据库再更新缓存”的方案

- 对于并发导致数据不一致的问题，可以使用分布式锁，确保同一时间只运行一个请求更新缓存
- 更新完缓存后，给缓存设置个过期时间，这样的话即便缓存不一致，也会很快过期

（2）针对一般性场景，可以采用【先删除缓存，再更新数据库】，再加上【延迟双删】的策略解决

- `延迟时间`需要大于请求 B 「从数据库读取数据 + 写入缓存」的时间
- 第一次删除是为了删除“脏读”，第二次删除是为了“最终一致性”

（3）上述的延迟双删策略中`延迟时间`很难确定，所以一般来说，还是使用【先更新数据库，再删除缓存】的方案，但是这个方案需要解决`删除缓存失败的问题`
有两种解决方案：第一种是加MQ重试，第二种是订阅MySQL binlog；两种方法都是异步操作缓存来确保缓存最终成功删除
...
压缩列表是由**连续内存块组成的顺序型数据结构**，压缩列表存在下列缺陷：

1. 查找复杂度高
除了查找第一个元素和最后一个元素（可以通过表头三个字段定位），其他元素都需要逐个查找，时间复杂度为O(N)；
2. 连锁更新
压缩列表在插入和修改某个元素时，如果空间不够，需要进行内存空间重新分配。如果插入的元素过大，会导致后续元素的`prevlen`占用空间都发生变化，导致每个元素的空间都要重新分配；

### 6.2 Bitmap压缩

使用 roaring bitmap会将32位整型拆分为高16位和低16位：

- 高16位会作为key存储到short[]中
- 低16位会作为value，存储到Container[]中的某个container中

![RoaringBitmap](/pic/RoaringBitmap.jpg)

keys数组保持有序，便于二分查找。对于`666666666`，首先获取其二进制：

```shell
# 高16位：10172
# 低16位：34474
0010 0111 1011 1100 | 1000 0110 1010 1010
```

通过二分查找在array中获取10172，如果不存在则创建，再去对应的container中找到低16位的34474（如果是位图container，则直接找到下标再置1；如果是数组container，则通过二分查找找到对应元素）
